...
Веб-интерфейс поднимается вместе со стендом и доступен на
[http://localhost:3000](http://localhost:3000) — это единственный сервис,
чей порт публикуется наружу.
...
ЕСИА, СБП, OZON и Wildberries в системе не подключены: каждая требует
договора или заявки от юридического лица. Что именно нужно получить,
как устроен протокол каждой, что писать в коде и чем это отлаживать —
в [EXTERNAL.md](EXTERNAL.md).
...
Проект целиком описан в одном PDF: назначение, архитектура, сервисы,
внешний API, развёртывание, переменные окружения, требования, безопасность
и принятые решения.
...
Документ собирается из исходников: текст, объясняющий решения, лежит
...
- [Реестр требований](docs/requirements.md) — что считается сделанным
  и по какому критерию. Источник истины о готовности.
- [Архитектура](docs/architecture/README.md) — контекст, контейнеры, границы
  сервисов и владение данными.
- [Решения (ADR)](docs/adr/README.md) — принятые архитектурные решения
  с условиями пересмотра.
- [Модель угроз](docs/threat-model.md) — активы, границы доверия,
  нарушители и принятые риски.
- [Производительность](docs/performance.md) — измеренный baseline
  и условия измерения.
- [CI/CD](docs/ci-cd.md) — что проверяется и как публикуются образы.
- [Отчёт по репозиторию](REPORT.md) — разбор состояния кода.
- [План работ](PLAN.md) и [задачи](TASKS.md).
...
Ниже — исходная постановка. Разнесённый по идентификаторам список
с критериями приёмки и фактическим статусом — в
[реестре требований](docs/requirements.md).
